...
#Importando os dados
df = pd.read_excel('Parâmetros_Andrews_Sim.xlsx')
...
def p_s1():
  Ci1 = data[1], data[2], 0
  s_p1 = data[3], data[4], data[5], data[6], data[7], data[8]
  s_p1 = tuple(s_p1)
...
def p_s2():
  Ci2 = data[1], data[2], 0
  s_p2 = data[3], data[4], data[5], data[6], data[7], data[8]
  s_p2 = tuple(s_p2)
...
if len(data) == 10:
  def p_s3():
    Ci3 = data[1], data[2], 0
    s_p3 = data[3], data[4], data[5], data[6], data[7], data[8], data[9]
    s_p3 = tuple(s_p3)
...
def save_s1():
  df_concents_dados_sim_andrews_s1 = pd.DataFrame(
    {'Tempo': t_s,'Cx': Cx1, 'Cs':Cs1, 'Cp':Cp1})
  df_concents_dados_produtividade_andrews_s1 = pd.DataFrame(
    {'Tempo': t_s[1:],'Produtividade Celualr': Produtividadex1, 'Produtividade Produto': Produtividadep1})
  df_concents_dados_mi_andrews_s1 = pd.DataFrame(
    {'Tempo': t_s,'Mi': mi1})
  with pd.ExcelWriter('Dados_simulação_andrews_associado.xlsx') as writer:
    df_concents_dados_sim_andrews_s1.to_excel(writer, sheet_name="Simulação")
    df_concents_dados_produtividade_andrews_s1.to_excel(writer, sheet_name="Produtividade")
    df_concents_dados_mi_andrews_s1.to_excel(writer, sheet_name="Mi")
    writer.save()

def save_s2():
  df_concents_dados_sim_andrews_s2 = pd.DataFrame(
    {'Tempo': t_s,'Cx': Cx2, 'Cs':Cs2, 'Cp':Cp2})
  df_concents_dados_produtividade_andrews_s2 = pd.DataFrame(
    {'Tempo': t_s[1:],'Produtividade Celualr': Produtividadex2, 'Produtividade Produto': Produtividadep2})
  df_concents_dados_mi_andrews_s2 = pd.DataFrame(
    {'Tempo': t_s,'Mi': mi2})
  with pd.ExcelWriter('Dados_simulação_andrews_não_associado.xlsx') as writer:
    df_concents_dados_sim_andrews_s2.to_excel(writer, sheet_name="Simulação")
    df_concents_dados_produtividade_andrews_s2.to_excel(writer, sheet_name="Produtividade")
    df_concents_dados_mi_andrews_s2.to_excel(writer, sheet_name="Mi")
    writer.save()

def save_s3():
  df_concents_dados_sim_andrews_s3 = pd.DataFrame(
    {'Tempo': t_s,'Cx': Cx3, 'Cs':Cs3, 'Cp':Cp3})
  df_concents_dados_produtividade_andrews_s3 = pd.DataFrame(
    {'Tempo': t_s[1:],'Produtividade Celualr': Produtividadex3, 'Produtividade Produto': Produtividadep3})
  df_concents_dados_mi_andrews_s3 = pd.DataFrame(
    {'Tempo': t_s,'Mi': mi3})
  with pd.ExcelWriter('Dados_simulação_andrews_parcialmente_associado.xlsx') as writer:
    df_concents_dados_sim_andrews_s3.to_excel(writer, sheet_name="Simulação")
    df_concents_dados_produtividade_andrews_s3.to_excel(writer, sheet_name="Produtividade")
    df_concents_dados_mi_andrews_s3.to_excel(writer, sheet_name="Mi")
    writer.save()