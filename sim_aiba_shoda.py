...
#Importando os dados
df = pd.read_excel('Parâmetros_Aiba_Sim.xlsx')
...
if len(data) == 10:
  ...
  def mi_aiba_s3():
    ...
    ax.set_title("Velocidade específica de crescimento - Aiba", weight='bold')
    ax.set_xlabel('Tempo (h)', weight='bold')
...
def save_s1():
  df_concents_dados_sim_aiba_s1 = pd.DataFrame(
    {'Tempo': t_s,'Cx': Cx1, 'Cs':Cs1, 'Cp':Cp1})
  df_concents_dados_produtividade_aiba_s1 = pd.DataFrame(
    {'Tempo': t_s[1:],'Produtividade Celualr': Produtividadex1, 'Produtividade Produto': Produtividadep1})
  df_concents_dados_mi_aiba_s1 = pd.DataFrame(
    {'Tempo': t_s,'Mi': mi1})
  with pd.ExcelWriter('Dados_simulação_aiba_associado.xlsx') as writer:
    df_concents_dados_sim_aiba_s1.to_excel(writer, sheet_name="Simulação")
    df_concents_dados_produtividade_aiba_s1.to_excel(writer, sheet_name="Produtividade")
    df_concents_dados_mi_aiba_s1.to_excel(writer, sheet_name="Mi")
    writer.save()

def save_s2():
  df_concents_dados_sim_aiba_s2 = pd.DataFrame(
    {'Tempo': t_s,'Cx': Cx2, 'Cs':Cs2, 'Cp':Cp2})
  df_concents_dados_produtividade_aiba_s2 = pd.DataFrame(
    {'Tempo': t_s[1:],'Produtividade Celualr': Produtividadex2, 'Produtividade Produto': Produtividadep2})
  df_concents_dados_mi_aiba_s2 = pd.DataFrame(
    {'Tempo': t_s,'Mi': mi2})
  with pd.ExcelWriter('Dados_simulação_aiba_não_associado.xlsx') as writer:
    df_concents_dados_sim_aiba_s2.to_excel(writer, sheet_name="Simulação")
    df_concents_dados_produtividade_aiba_s2.to_excel(writer, sheet_name="Produtividade")
    df_concents_dados_mi_aiba_s2.to_excel(writer, sheet_name="Mi")
    writer.save()

def save_s3():
  df_concents_dados_sim_aiba_s3 = pd.DataFrame(
    {'Tempo': t_s,'Cx': Cx3, 'Cs':Cs3, 'Cp':Cp3})
  df_concents_dados_produtividade_aiba_s3 = pd.DataFrame(
    {'Tempo': t_s[1:],'Produtividade Celualr': Produtividadex3, 'Produtividade Produto': Produtividadep3})
  df_concents_dados_mi_aiba_s3 = pd.DataFrame(
    {'Tempo': t_s,'Mi': mi3})
  with pd.ExcelWriter('Dados_simulação_aiba_parcialmente_associado.xlsx') as writer:
    df_concents_dados_sim_aiba_s3.to_excel(writer, sheet_name="Simulação")
    df_concents_dados_produtividade_aiba_s3.to_excel(writer, sheet_name="Produtividade")
    df_concents_dados_mi_aiba_s3.to_excel(writer, sheet_name="Mi")
    writer.save()